...
export async function handleAbortChat(host: ChatHost) {
  ...
  host.chatMessage = "";
  await abortChatRun(host as unknown as GoBotApp);
}
...
async function sendChatMessageNow(
  ...
  const ok = await sendChatMessage(host as unknown as GoBotApp, message);
...
export async function refreshChat(host: ChatHost) {
  await Promise.all([
    loadChatHistory(host as unknown as GoBotApp),
