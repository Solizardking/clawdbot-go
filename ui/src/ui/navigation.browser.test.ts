...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/gobot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/gobot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/gobot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__GOBOT_CONTROL_UI_BASE_PATH__ = "/gobot";
    const app = mountApp("/gobot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/gobot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/gobot/sessions");
  });
