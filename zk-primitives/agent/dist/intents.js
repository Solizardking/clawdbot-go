 ...
 */
export function routeIntent(text, _agent, ctx = {}) {
    const candidates = [];
    // ----- attest-model -----
 ...
 */
export async function dispatchRoute(route, agent) {
    switch (route.action) {
        case "attestModel": {
            const { modelHash, payloadCommitment, context, proofPath } = route.args;
            if (!modelHash)
                throw new Error("attestModel intent requires a modelHash (hex).");
            if (!payloadCommitment)
                throw new Error("attestModel intent requires a payloadCommitment (hex).");
            if (!proofPath)
                throw new Error("attestModel intent requires a proofPath (JSON file).");
            const proof = await ClawdZkAgent.loadProof(proofPath);
            const hexToBytes = (s) => Uint8Array.from(s.replace(/^0x/i, "").match(/.{1,2}/g)?.map((b) => parseInt(b, 16)) ?? []);
            return agent.attestModel({
                modelHash: hexToBytes(modelHash),
                payloadCommitment: hexToBytes(payloadCommitment),
                proof,
                context,
            });
        }
        case "commitEncryptedState": {
            const { modelHash, ciphertextCommitment, stateVersion, proofPath } = route.args;
            if (!ciphertextCommitment)
                throw new Error("commitEncryptedState intent requires a ciphertextCommitment (hex).");
            if (!proofPath)
                throw new Error("commitEncryptedState intent requires a proofPath (JSON file).");
            const proof = await ClawdZkAgent.loadProof(proofPath);
            const hexToBytes = (s) => Uint8Array.from(s.replace(/^0x/i, "").match(/.{1,2}/g)?.map((b) => parseInt(b, 16)) ?? []);
            return agent.commitEncryptedState({
                modelHash: (modelHash ? hexToBytes(modelHash) : new Uint8Array(32)),
                ciphertextCommitment: hexToBytes(ciphertextCommitment),
                stateVersion,
                proof,
        ...
        case "verifyProof": {
            const { proofPath } = route.args;
            if (!proofPath)
                throw new Error("verifyProof intent requires a proofPath (JSON file).");
            const proof = await ClawdZkAgent.loadProof(proofPath);
            return agent.verifyProof({ proof });
        }
        case "computeNullifier": {
            const { context } = route.args;
        ...
        case "help":
            return HELP_TEXT;
        default:
            throw new Error(`Unhandled action: ${route.action}`);
    }
}
