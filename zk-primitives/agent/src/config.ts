...
function resolveProgramId(raw: string | undefined): PublicKey {
  ...
  try {
    ...
    throw new Error(
      `Invalid ZK_SHARK_PROGRAM_ID/GOBOT_ZK_PROGRAM_ID: ${raw}. Expected a base58 pubkey or one of: ${Object.keys(KNOWN_PROGRAM_IDS).join(", ")}.`,
    );
  }
}
...
export function loadAgentConfig(
  ...
  if (!rpcUrl) {
    throw new Error(
      "ZK_SHARK_RPC_URL is not set. Add it to your environment, or keep using legacy GOBOT_ZK_RPC_URL.",
    );
  }
